...
import com.mml.core.voice.core.recog.IStatus.Companion.WHAT_MESSAGE_STATUS
...
class MessageStatusRecogListener(private val handler: Handler?) : StatusRecogListener() {
    ...
    override fun onAsrPartialResult(results: Array<String>?, recogResult: RecogResult) {
        sendStatusMessage(
            ...
            "临时识别结果，结果是“" + results!![0] + "”；原始json：" + recogResult.origalJson
        )
    ...
    override fun onAsrFinalResult(results: Array<String>?, recogResult: RecogResult) {
        ...
        sendStatusMessage(
            SpeechConstant.CALLBACK_EVENT_ASR_PARTIAL,
            message + "；原始json：" + recogResult.origalJson
        )
        if (speechEndTime > 0) {
    ...
    override fun onAsrFinishError(
        ...
        var message = "【asr.finish事件】识别错误, 错误码：$errorCode ,$subErrorCode ; $descMessage"
    ...
    private fun sendMessage(message: String, what: Int = WHAT_MESSAGE_STATUS, highlight: Boolean = false) {
        var message = message
